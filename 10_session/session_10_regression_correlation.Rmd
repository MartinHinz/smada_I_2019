...
subtitle: "10 - Regression and Correlation"
author: "Martin Hinz"
...
hmuens$'var(FL)' <- (hmuens$FL - mean(hmuens$FL))^2
...
$a_{min} = 71 - `r (as.numeric(sum_hmuens[5]) / as.numeric(sum_hmuens[6]))` * 44$

$a_{min} = `r 71 - (as.numeric(sum_hmuens[5]) / as.numeric(sum_hmuens[6])) * 44`$
]
]
...
.pull-right[
```{r echo=F}
plot(FL~Length, data=data.frame(head(muensingen)), ylim = c(0,90), asp=1)
abline(
